        ...
        if (!this.props.mouseEventToElement) {
            ...
            let dropDowns = this.props.options.map(function (option) {
                ...
                return <div style={paddingStyle}>
                    {option.lable}
                    <Dropdown onCustomTypeSelect={this.onCustomTypeSelect.bind(this)}
                              enter={this.state.mouseStates[0].enter} leave={this.state.mouseStates[0].leave}
                              noInitial={option.noInitial} onItemSelect={this.onItemSelect.bind(this)} ref={option.ref}
                              items={option.items} title={option.title}/>
                </div>

            }.bind(this));

            return (
                <td ref={this.props.ref} onMouseOver={this.handleMouseEnter.bind(this, 0)}
                    onMouseOut={this.handleMouseLeave.bind(this, 0)}>
                    {dropDowns}
                </td>
            );
            ...
            let dropDowns = this.props.options.map(function (option, i) {
                //let styleForDiv={{}};
                return <div style={paddingStyle} onMouseEnter={this.handleMouseEnter.bind(this, i)}
                            onMouseLeave={this.handleMouseLeave.bind(this, i)}>
                    {option.lable}
                    <Dropdown onCustomTypeSelect={this.onCustomTypeSelect.bind(this)}
                              enter={this.state.mouseStates[i].enter} leave={this.state.mouseStates[i].leave}
                              noInitial={option.noInitial} onItemSelect={this.onItemSelect.bind(this)} ref={option.ref}
                              items={option.items} title={option.title}/>
                </div>
